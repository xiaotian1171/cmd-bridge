...
2. 本轮做"首次从旧版切换到新版"的验证时，**又抓出三个真实代码问题**（A12、A13、A14）。A12 会在 TLS 关闭时把**同机另一套桥的网关**当成"本桥旧实例"强杀；A14 是守护进程自身的身份标记在**生产启动方式**下不生效（`/proc/<pid>/environ` 只反映 exec 时刻的环境），导致 `stop.sh` 认不出守护、留下一个停不掉的守护 —— 都已修复并有测试。
...
| **A14** | 守护进程**自身的身份标记在生产启动方式下不生效**：`/proc/<pid>/environ` 只反映 exec 那一刻的环境，脚本内部的 `export BRIDGE_OWNER` 不会出现在**守护自己**的 environ 里（它拉起的子进程正常，因为子进程是带着 export 后的环境 exec 的）。线上用 `setsid nohup bash keepalive.sh` 启动时环境里没有任何 `BRIDGE_*`，于是守护既不算 `owned`、也没有旧实例证据（cmdline 里是脚本目录而不是 `BRIDGE_HOME`，它也不监听端口、不持有 sock）→ **`stop.sh` 漏掉守护，留下一个每 5s 抢重启的守护**。测试之所以曾经全绿，是因为测试给守护预置了 `BRIDGE_HOME`，走的是"旧实例 P2"路径 | 线上首切实测：`stop.sh` 停掉网关/中枢/引擎/隧道并释放端口后，旧守护（PID 3361056）仍在运行，`scoped_pids` 对它返回空、`stop_role keepalive` 输出"未在运行" | `keepalive.sh` 在定义完 home/sock 后**用带标记的环境重新 exec 自己**（`exec bash "$SELF"`），让守护自身的 environ 带上 `BRIDGE_OWNER`/`DC_HUB_SOCK`；`stop.sh` 对被漏掉的无标记守护**打印警告并要求人工确认**（只提示、不误杀，因为同机另一套桥可能复用同一份脚本目录）；t8 新增 7 项断言（生产启动方式的守护可被 owned 口径识别并可被 `stop.sh` 停掉、无标记守护被提示且不被误杀） |
...
- 新 `stop.sh` **不依赖 `BRIDGE_KILL_LEGACY=1`**，就能精确停掉这套桥的旧实例：旧网关（按本桥端口）、旧中枢（按 env/sock）、旧 session 转发进程、**旧 keepalive 守护**。
  - 注意这里旧守护能被认出来，是靠它 environ 里的 `BRIDGE_HOME`（测试按这种方式给它设了环境）。**生产上不是这样**：线上守护是 `setsid nohup bash keepalive.sh` 起的，环境里没有任何 `BRIDGE_*`，实测认不出来 —— 这就是 A14，已修（守护自我 re-exec）并对"已存在的无标记守护"加了人工确认提示。
...
| `test/t8-scope.cjs` | 停止/重启作用域、抢重启、**守护自身身份标记（生产启动方式）** | **29 / 0** |
...
旧实例：本轮 t11 证明新 `stop.sh` **不需要 `BRIDGE_KILL_LEGACY=1`** 也能精确停掉改造前启动的旧实例（含旧守护，条件是守护 environ 里能查到本桥 home），并且不会误停同机另一套桥与同名诱饵。
**首切注意（A14）**：线上那个**改造前就已存在、环境里没有任何 `BRIDGE_*`** 的守护，凭本桥独占资源无法判定归属，`stop.sh` 只会打印警告。首次切换时必须人工确认并 `kill` 掉它，否则它会继续每 5s 拉起桥、并且以后都停不掉。改用新 `keepalive.sh` 启动后，守护自身带标记，后续 `stop.sh` 能按身份精确停止。
...
4. **确认停干净**：`ss -ltnp | grep <端口>` 无输出；`pgrep -af dc-hub-client | wc -l` 为 0。
   - **额外一步（首切必须）**：`pgrep -af keepalive.sh` 看看有没有 `stop.sh` 漏掉的守护。线上首切实测有一个（改造前启动、environ 里没有任何 `BRIDGE_*`）。确认它是本桥的（`ps -o pid,lstart,cmd -p <pid>`）后 `kill <pid>`，否则它会每 5s 把桥拉起来。
5. **起新桥**：`bash start.sh`，确认 `/healthz` 200，客户端做一次 `initialize → tools/list → tools/call`。
   - 起桥时要确认入口路径 token **没有被换掉**（线上 `run_env` 里的 token 与实际在用的 token 文件曾经不一致，`source run_env` 会改掉入口；本次切换刻意不 `source` 它，让 `start.sh` 走文件回退）。
   - 另外确认新起的网关命令行里带上 `--sessionTimeout <目标毫秒>` 与 `--healthEndpoint /healthz`。
...
> 报告第一版停在代码与报告阶段。2026-09-26 按用户授权执行了线上切换，记录见第七节。

---

## 七、线上首切记录（2026-09-26，已授权执行）

顺序：只读勘查 → 备份 → 原地覆盖代码 → `stop.sh` → 人工处理漏掉的旧守护 → `start.sh` → 新守护 → 从公网入口验证。

1. **勘查**：线上运行实例的 `BRIDGE_MODE=full`、`BRIDGE_TLS=0`、隧道 cloudflare；入口路径 token 用的是 `~/.bridge/token` 文件里的那个，而 `~/.bridge/run_env` 里写的是另一个（两者不一致，具体值不外发）。**因此启动时刻意不 `source run_env`**，让 `start.sh` 走文件回退，入口 token 保持不变。
2. **备份**：`~/backup-cmd-bridge-20260926.tar.gz`（31MB，排除 `dc-hub.sock`），校验可读。
3. **切代码**：11 个文件原地覆盖，逐个 `md5sum` 与副本一致（`start.sh` `29195ad2…` 等）。
4. **`stop.sh`**：输出显示精确识别并停掉旧实例 —— 网关 `3380125`、中枢 `3380080`、引擎 `3380089 3380090 3380097`、`cloudflared` `3380142`，并输出"端口已释放"；同机无其它桥受影响。全程**未设 `BRIDGE_KILL_LEGACY`**。
5. **发现的第一个问题（→ A14）**：改造前启动的旧守护 `3361056` 没有被停掉（它不是 owned，也没有任何指向本桥的 env/sock/端口证据）。它每 5s 检查一次，看到 `stopping` 标志时跳过一次重启，标志被清掉后就把桥拉了起来。本次**人工 `kill`** 处理。
6. **新桥起来（由新 `start.sh` 拉起）**：网关命令行里出现 `--sessionTimeout 1800000`（30 分钟，来自 `~/.bridge/session_timeout` 存档）与 `--healthEndpoint /healthz`；入口 token 仍是在用的那个（未变）；隧道进程带 `BRIDGE_OWNER`。说明新 `start.sh` 的"存档回退 + 尾部写档"在线上生效。
7. **守护替换**：`kill` 旧守护后用新 `keepalive.sh` 重启；随后确认守护自身 environ 带 `BRIDGE_OWNER`、`stop.sh` 的 owned 口径能认出它。
8. **观察项（30 分钟档）**：`keepalive.log` 无故重启次数、`dc-hub-client` 数量是否收敛、`sg.log` 中 `No connection established` 次数、客户端是否出现"失败一次再成功"。
9. **回滚**：解包备份 → `stop.sh` / `start.sh` 各一次（纯文件层）。

> 未做：未改 `run_env`（其中 token 与线上实际不一致，先记录不动）、未改 Jev 项目、未删任何文件。
