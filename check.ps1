...
#
# 检查 node / cloudflared / ngrok / MCP 组件是否就绪、进程是否在跑。
...
Write-Host "ngrok: $(if (Test-Path $NgExe) { $NgExe } else { '未安装 -> 先跑 install.ps1' })"
Write-Host "token: $(if (Test-Path $TokenFile) { (Get-Content $TokenFile -Raw).Trim().Substring(0, 8) + '...' } else { '未生成 -> start.ps1 首次运行时自动生成' })"
Write-Host ""
Write-Host "进程:"
Get-CimInstance Win32_Process | Where-Object { $_.CommandLine -match "supergateway|desktop-commander|filter-proxy|cloudflared tunnel|ngrok http" } | ForEach-Object { Write-Host "  PID $($_.ProcessId)  $($_.Name)" }
Write-Host ""
